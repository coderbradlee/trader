...
class parameter_analysis(object):
    ...
    def get_backtest_data(self,
        ...
        # 调运排队回测函数，传递对应参数
        self.run_backtest(algorithm_id=algorithm_id, running_max=running_max, start_date=start_date, end_date=end_date, frequency=frequency, initial_cash=initial_cash, param_names=param_names, param_values=param_values, python_version=python_version, use_credit=use_credit)
    ...
    def plot_returns(self):
        ...
        # 设定benchmark曲线并标记
        ax.plot(list(range(len(self.benchmark_returns))), self.benchmark_returns, label='benchmark', c='k',
                linestyle='--')
    ...
    def plot_excess_returns(self):
        ...
        # 设定benchmark曲线并标记
        ax.plot(list(range(len(self.benchmark_returns))), [0] * len(self.benchmark_returns), label='benchmark', c='k',
                linestyle='--')
        ticks = [int(x) for x in np.linspace(0, len(self.dates) - 1, 11)]
        plt.xticks(ticks, [self.dates[i] for i in ticks])
        # 设置图例样式
    ...
    def plot_log_returns(self):
        ...
        # 设定benchmark曲线并标记
        ax.plot(list(range(len(self.benchmark_returns))), [log(x + 1) for x in self.benchmark_returns],
                label='benchmark', c='k', linestyle='--')
        ticks = [int(x) for x in np.linspace(0, len(self.dates) - 1, 11)]
        plt.xticks(ticks, [self.dates[i] for i in ticks])
        # 设置图例样式
        ax.legend(loc=2, fontsize=10)
...
alrithomid = "cd5af1ca8daf0eaa17829ac0dc2d53c0"
# user="user"
# password="password"
...
def backtest():
    ...
    pa = parameter_analysis(alrithomid)
    # t = jqdatasdk.get_index_stocks("000016.XSHG", "2020-07-28")
    pa.get_backtest_data(algorithm_id=None, benchmark_id=None, file_name='results.pkl', running_max=10, start_date='2019-01-01', end_date='2020-07-28', frequency='day', initial_cash='1000000', param_names=['stocksnum', 'period'], param_values=[[7, 8], [8, 9]])
    pa.read_backtest_data(file_name='results.pkl')
    # pa.params_df
    # pa.evaluations_df
    # pa.plot_returns()
    # # 8 超额收益率图
