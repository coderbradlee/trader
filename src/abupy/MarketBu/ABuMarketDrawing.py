...
def _do_plot_candle(date, p_open, high, low, close, volume, view_index, symbol, day_sum, save, minute):
    ...
    if not g_only_draw_price:
        ...
        fig, (ax1, ax2) = plt.subplots(2, sharex=False, figsize=(20, 12))
    else:
        # 只绘制价格
    ...
    if not g_only_draw_price:
        ...
        if view_index is not None:
            ...
            for v, csColor in zip(view_index, itertools.cycle(K_PLT_MAP_STYLE)):
                ...
                # print("csColor", csColor)
                print("label", label)
                # print("v", str(v))
                ax1.plot(v, 0, 'ro', markersize=12, markeredgewidth=0.5, markerfacecolor='None',
                         markeredgecolor=csColor, label=label)
            # plt.legend(loc='best')
            plt.legend(['ts'], loc='best')
